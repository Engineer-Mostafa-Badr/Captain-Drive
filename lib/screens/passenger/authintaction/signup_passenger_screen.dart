...
import 'package:captain_drive/screens/passenger/authintaction/passenger_auth/passenger_signup_cubit.dart';
...
class _SignUpPassengerScreenState extends State<SignUpPassengerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => PassengerSignUpCubit(),
      child: Scaffold(
        backgroundColor: backGroundColor,
        body: BlocConsumer<PassengerSignUpCubit, PassengerSignUpState>(
          listener: (context, state) {
            if (state is PassengerSignUpSuccess) {
              if (state.passengerRegisterModel.status) {
                print('sjjjjjsjjsjsjsjsjsjsjsjsjsjjsjsjsjsjs');
                print(state.passengerRegisterModel.data?.token);
                print(state.passengerRegisterModel.message);

                CacheHelper.saveData(
                        key: 'token',
                        value: state.passengerRegisterModel.data?.token)
                    .then((value) {
                  userToken = state.passengerRegisterModel.data?.token;
                ...
                showToast(
                    message: state.passengerRegisterModel.message,
                    color: Colors.red);
              }
            }
            if (state is PassengerSignUpFailure) {
              showToast(
            ...
            if (state is PassengerSignUpWithGoogleSuccess) {
              Navigator.pushReplacement(context,
                  MaterialPageRoute(builder: (context) => const LayoutScreen()));
            }

            if (state is PassengerSignUpWithGoogleFailure) {
              showToast(
                message: state.message,
                color: Colors.red,
              );
            ...
            if (state is PassengerSignUpWithFacebookSuccess) {
              Navigator.pushReplacement(context,
                  MaterialPageRoute(builder: (context) => const HomeScreen()));
            }

            if (state is PassengerSignUpWithFacebookFailure) {
              showToast(
                message: state.message,
                color: Colors.red,
              );
                      ...
                      child: Form(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            ConditionalBuilder(
                              condition: state is! PassengerSignUpLoading,
                              builder: (context) => customButton(
                                  ...
                                  function: () {
                                    ...
                                    PassengerSignUpCubit.get(context)
                                        .userRegister(
                                            context: context,
                                            name: name!,
                                            email: email!,
